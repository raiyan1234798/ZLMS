...
export default function CompanyAdminOverview() {
    ...
    useEffect(() => {
        const fetchDashboardData = async () => {
            ...
            try {
                ...
                uSnap.forEach(d => {
                    const ud: any = { id: d.id, ...d.data() };
                    uData.push(ud);
                    if (ud.role === 'COMPANY_ADMIN' && !adminFound) adminFound = ud;
                });
    ...
    if (!company) {
        return <div style={{ padding: '40px', textAlign: 'center' }}>Company not found.</div>;
    }
                        ...
                        {companyCourses.map((course) => {
                            ...
                            return (
                                <tr key={course.id} style={{ borderTop: '1px solid var(--border)' }}>
                                    ...
                                    <td style={{ padding: '14px 24px', color: 'var(--text-muted)', fontSize: '0.9rem' }}>{course.modules?.length || 0}</td>
