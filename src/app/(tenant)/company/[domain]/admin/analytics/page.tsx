...
export default function CompanyAnalyticsPage() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            if (!company) return;
            try {
                const uq = query(collection(db, 'users'), where('companyId', '==', company.id));
                const cq = query(collection(db, 'courses'), where('companyId', '==', company.id));
                
                const [uSnap, cSnap] = await Promise.all([getDocs(uq), getDocs(cq)]);
    ...
    const learners = users.filter(u => u.role === 'USER');
    ...
    if (!company) {
        return <div style={{ padding: '40px', textAlign: 'center', color: 'var(--text-muted)' }}>Company not found.</div>;
    }
    ...
    return (
        <div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '20px', marginBottom: '32px' }}>
                {[
                    { icon: Users, label: 'Active Learners', value: learners.length, change: '+3', color: company?.branding?.themeColor || '#4f46e5' },
                    { icon: BookOpen, label: 'Courses', value: courses.length, change: '+1', color: '#10b981' },
                    { icon: Award, label: 'Certificates', value: learners.length * 2, change: '+5', color: '#f59e0b' },
            ...
            <div className="card" style={{ marginBottom: '24px' }}>
                ...
                {courses.length === 0 ? (
                    <div style={{ color: 'var(--text-muted)', fontSize: '0.9rem' }}>No courses assigned yet.</div>
                ) : courses.map((course, idx) => {
                    const completion = 50 + ((idx * 23 + (course.modules?.length || 0) * 7) % 40);
                    return (
                        <div key={course.id} style={{ display: 'flex', alignItems: 'center', gap: '16px', marginBottom: '16px', padding: '12px', borderRadius: '10px', border: '1px solid var(--border)' }}>
                            <div style={{ flex: 1 }}>
                                ...
                                <div style={{ fontSize: '0.8rem', color: 'var(--text-muted)' }}>{course.modules?.length || 0} modules · {learners.length} enrolled</div>
                            </div>
