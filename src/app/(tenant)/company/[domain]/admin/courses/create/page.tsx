...
import { ArrowLeft, Save, Plus, Trash2, Video, FileQuestion, PlayCircle, Link as LinkIcon, UploadCloud, List, Type, Check, FileCheck, MonitorPlay, LayoutGrid, BookOpen, Layers } from 'lucide-react';
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            if (companyId !== 'unknown') {
                await NotificationService.notifyNewCourse(courseTitle, companyId, companyName);
                await ActivityService.logCourseCreated(courseTitle, companyId, companyName);
            }
                        ...
                        <>
                            ...
                            <div style={cardS}>
                                <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                                    {activeLesson.type === 'video' ? <Video size={20} color="var(--tenant-primary, #7c3aed)" /> : 
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                            {lessons.map(item => {
                                ...
                                <div key={item.id} onClick={() => setSelectedLessonId(item.id)} style={{
                                    ...
                                    borderRadius: '12px', 
                                    border: `1px solid ${selectedLessonId === item.id ? 'var(--tenant-primary, #7c3aed)' : isModule ? '#333' : 'var(--border)'}`,
                                    background: selectedLessonId === item.id ? 'var(--tenant-primary, #7c3aed)08' : isModule ? '#1a1a1a' : 'var(--surface)',
                                    ...
                                    <div style={{ display: 'flex', alignItems: 'center', gap: '10px', overflow: 'hidden' }}>
                                        <div style={{
                                            ...
                                            background: selectedLessonId === item.id || isModule ? 'var(--tenant-primary, #7c3aed)' : 'var(--background)',
