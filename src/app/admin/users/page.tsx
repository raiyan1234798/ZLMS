...
import { Search, UserPlus, Shield, GraduationCap, Users, BookOpen, Edit3, Trash2, X, Check, MoreVertical, Clock, CheckCircle, XCircle, UserCheck, RefreshCw } from 'lucide-react';
...
type Status = 'ACTIVE' | 'PENDING_APPROVAL' | 'PRE_APPROVED' | 'SUSPENDED' | 'REJECTED';
...
export default function AllUsersPage() {
    ...
    const [newStatus, setNewStatus] = useState<Status>('PRE_APPROVED');
    ...
    // Fetch live companies from Firestore
    useEffect(() => {
        const fetchCompanies = async () => {
            try {
                const snap = await getDocs(collection(db, 'companies'));
                const cos: LiveCompany[] = [];
                snap.forEach(d => {
                    const data = d.data();
                    cos.push({ id: d.id, name: data.name, subdomain: data.subdomain, themeColor: data.themeColor, status: data.status });
                });
                setLiveCompanies(cos);
            } catch (err) {
                console.error('Error fetching companies:', err);
            }
        };
        fetchCompanies();
        const unsub = onSnapshot(collection(db, 'companies'), fetchCompanies);
        return () => unsub();
    }, []);
    ...
    const handleEditUser = async () => {
        ...
        try {
            ...
            setPlatformUsers(prev => prev.map(u =>
                u.id === editingUser.id ? { ...u, role: editRole, name: editName, status: editStatus } : u
            ));
            setEditingUser(null);
        } catch (error) {
    ...
    const handleApproveUser = async (user: PlatformUser, role: Role = 'USER') => {
        try {
            ...
            setPlatformUsers(prev => prev.map(u =>
                u.id === user.id ? { ...u, status: 'ACTIVE' as Status, role } : u
            ));
        } catch (error) {
    ...
    const handleRejectUser = async (user: PlatformUser) => {
        try {
            await updateDoc(doc(db, 'users', user.id), {
                status: 'REJECTED',
            });
            setPlatformUsers(prev => prev.map(u =>
                u.id === user.id ? { ...u, status: 'REJECTED' as Status } : u
            ));
        } catch (error) {
    ...
    const handlePreApproveUser = async (user: PlatformUser, role: Role = 'USER') => {
        try {
            await updateDoc(doc(db, 'users', user.id), {
                status: 'PRE_APPROVED',
                role: role,
            ...
            setPlatformUsers(prev => prev.map(u =>
                u.id === user.id ? { ...u, status: 'PRE_APPROVED' as Status, role } : u
            ));
        } catch (error) {
    ...
    const filtered = platformUsers
        .filter(u => filter === 'ALL' || (filter === 'PENDING' ? (u.status === 'PENDING_APPROVAL' || u.status === 'PRE_APPROVED') : u.role === filter))
        .filter(u => {
    ...
    const pendingCount = platformUsers.filter(u => u.status === 'PENDING_APPROVAL' || u.status === 'PRE_APPROVED').length;
    const activeCount = platformUsers.filter(u => u.status === 'ACTIVE').length;
    ...
    const statusColor = (status: string) => {
        switch (status) {
            ...
            case 'PENDING_APPROVAL': return { bg: '#fef3c7', color: '#d97706' };
            case 'PRE_APPROVED': return { bg: '#e0e7ff', color: '#4f46e5' };
            case 'SUSPENDED': return { bg: '#fef2f2', color: '#ef4444' };
        ...
        <div>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '32px', flexWrap: 'wrap', gap: '16px' }}>
                <div>
                    ...
                    <p style={{ color: 'var(--text-muted)' }}>
                        {platformUsers.length} users across all companies.
                        {pendingCount > 0 && <span style={{ marginLeft: '8px', color: '#f59e0b' }}>• {pendingCount} pending approval</span>}
                    </p>
                </div>
                <div style={{ display: 'flex', gap: '12px' }}>
                    <button onClick={fetchUsers} className="btn-secondary" style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: '16px', marginBottom: '28px' }}>
                {[
                    ...
                    { icon: Clock, label: 'Pending', value: pendingCount, color: '#f59e0b', bg: '#fffbeb' },
                    { icon: CheckCircle, label: 'Active', value: activeCount, color: '#10b981', bg: '#ecfdf5' },
                ].map((s, i) => {
                    const Icon = s.icon;
                    return (
            ...
            <div style={{ display: 'flex', gap: '12px', marginBottom: '20px', flexWrap: 'wrap' }}>
                ...
                {['ALL', 'PENDING', 'SUPER_ADMIN', 'COMPANY_ADMIN', 'TRAINER', 'USER'].map(r => (
                    <button key={r} onClick={() => setFilter(r)} className={filter === r ? 'btn-primary' : 'btn-secondary'} style={{ padding: '8px 16px', fontSize: '0.85rem' }}>
                        {r === 'ALL' ? 'All' : r === 'PENDING' ? 'Pending' : r === 'SUPER_ADMIN' ? 'Super Admins' : r === 'COMPANY_ADMIN' ? 'Admins' : r === 'TRAINER' ? 'Trainers' : 'Learners'}
                    </button>
                ))}
            </div>
                    ...
                    <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                        <thead>
                            <tr style={{ background: 'var(--background)' }}>
                                ...
                                <th style={{ padding: '12px 24px', textAlign: 'left', fontSize: '0.8rem', color: 'var(--text-muted)', fontWeight: 500 }}>Actions</th>
                            </tr>
                        </thead>
                        <tbody>
                            {filtered.map(user => {
                                const company = liveCompanies.find(c => c.id === user.companyId);
                                const rc = roleColor(user.role);
                                const sc = statusColor(user.status || 'ACTIVE');
                                const isPending = user.status === 'PENDING_APPROVAL' || user.status === 'PRE_APPROVED';
                                return (
                                    <tr key={user.id} style={{ borderTop: '1px solid var(--border)', background: isPending ? '#fffbeb' : 'transparent' }}>
                                        <td style={{ padding: '14px 24px' }}>
                                            <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                                                <div style={{ width: '36px', height: '36px', borderRadius: '50%', background: company?.themeColor || '#4f46e5', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'white', fontSize: '0.8rem', fontWeight: 600 }}>
                                        ...
                                        <td style={{ padding: '14px 24px' }}>
                                            <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                                                <div style={{ width: '24px', height: '24px', borderRadius: '6px', background: company?.themeColor || '#ccc', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'white', fontSize: '0.65rem', fontWeight: 600 }}>
                                        ...
                                        <td style={{ padding: '14px 24px' }}>
                                            <span style={{ padding: '3px 10px', borderRadius: '999px', fontSize: '0.75rem', fontWeight: 500, background: sc.bg, color: sc.color }}>
                                        ...
                                        <td style={{ padding: '14px 24px', textAlign: 'right', position: 'relative' }}>
                                            {isPending && (
                                                <div style={{ display: 'flex', gap: '8px', justifyContent: 'flex-end' }}>
                                                    <button onClick={() => handleApproveUser(user, 'USER')} style={{ padding: '6px 12px', background: '#10b981', color: 'white', borderRadius: '8px', border: 'none', fontSize: '0.8rem', fontWeight: 500, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '4px' }}>
                                                        <CheckCircle size={14} /> Approve
                                                    </button>
                                                    <button onClick={() => handleRejectUser(user)} style={{ padding: '6px 12px', background: '#ef4444', color: 'white', borderRadius: '8px', border: 'none', fontSize: '0.8rem', fontWeight: 500, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '4px' }}>
                                                ...
                                                <div ref={openMenuId === user.id ? menuRef : null} style={{ position: 'relative', display: 'inline-block' }}>
                                                    ...
                                                    {openMenuId === user.id && (
                                                        <div style={{
                                                            ...
                                                            boxShadow: '0 10px 40px rgba(0,0,0,0.12)', zIndex: 100, minWidth: '180px',
                                                            overflow: 'hidden', animation: 'fadeInUp 0.2s ease'
                                                        }}>
                                                            <button onClick={() => { setEditingUser(user); setEditRole(user.role); setEditName(user.name || ''); setEditStatus(user.status || 'ACTIVE'); setOpenMenuId(null); }} style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '10px 16px', width: '100%', fontSize: '0.85rem', color: 'var(--foreground)', textAlign: 'left' }}>
                                                                <Edit3 size={14} color="#4f46e5" /> Edit User
                                                            </button>
                                                            <button onClick={() => { handlePreApproveUser(user, 'USER'); setOpenMenuId(null); }} style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '10px 16px', width: '100%', fontSize: '0.85rem', color: '#4f46e5', textAlign: 'left' }}>
                                                                <UserCheck size={14} /> Pre-Approve
                                                            </button>
                                                            <div style={{ height: '1px', background: 'var(--border)' }} />
                                                            <button onClick={() => { setDeletingUser(user); setOpenMenuId(null); }} style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '10px 16px', width: '100%', fontSize: '0.85rem', color: '#ef4444', textAlign: 'left' }}>
                ...
                <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, backdropFilter: 'blur(4px)' }} onClick={() => setEditingUser(null)}>
                    <div className="animate-fade-in-up" onClick={e => e.stopPropagation()} style={{ background: 'var(--surface)', borderRadius: '20px', padding: '32px', width: '100%', maxWidth: '440px', boxShadow: '0 25px 50px rgba(0,0,0,0.2)' }}>
                        ...
                        <div style={{ marginBottom: '24px' }}>
                            <label style={{ display: 'block', fontSize: '0.85rem', fontWeight: 500, marginBottom: '8px', color: 'var(--text-muted)' }}>Status</label>
                            <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
                                {(['ACTIVE', 'SUSPENDED', 'REJECTED'] as Status[]).map(s => {
                                    ...
                                    return (
                                        <button key={s} onClick={() => setEditStatus(s)} style={{
                                            ...
                                            background: selected ? sc.color : 'var(--background)',
                                            color: selected ? 'white' : sc.color,
                                            border: `2px solid ${selected ? sc.color : 'var(--border)'}`,
                                        }}>
                                            {s === 'ACTIVE' ? '✓ Active' : s === 'SUSPENDED' ? '✕ Suspended' : '✕ Rejected'}
                                        </button>
                                    );
                                })}
                            </div>
                ...
                <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, backdropFilter: 'blur(4px)' }} onClick={() => setShowAddModal(false)}>
                    <div className="animate-fade-in-up" onClick={e => e.stopPropagation()} style={{ background: 'var(--surface)', borderRadius: '20px', padding: '32px', width: '100%', maxWidth: '480px', boxShadow: '0 25px 50px rgba(0,0,0,0.2)' }}>
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                            ...
                            <div>
                                <label style={{ display: 'block', fontSize: '0.85rem', fontWeight: 500, marginBottom: '6px', color: 'var(--text-muted)' }}>Initial Status</label>
                                <div style={{ display: 'flex', gap: '8px' }}>
                                    {(['PRE_APPROVED', 'ACTIVE'] as Status[]).map(s => {
                                        const sc = statusColor(s);
                                        ...
                                        return (
                                            <button key={s} onClick={() => setNewStatus(s)} style={{
                                                ...
                                                {s === 'PRE_APPROVED' ? '✓ Pre-Approved' : '✓ Active'}
                                            </button>
                                        );
                                    })}
                                </div>
