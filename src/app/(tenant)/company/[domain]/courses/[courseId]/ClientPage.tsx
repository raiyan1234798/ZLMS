...
export default function CoursePlayer() {
    ...
    return (
        <div style={{ minHeight: '100vh', background: '#0f0f0f', color: 'white' }}>
            {/* Top Bar */}
            <div style={{ padding: '12px 24px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', borderBottom: '1px solid #222' }}>
                <Link href={`/company/${domain}/dashboard`} style={{ display: 'flex', alignItems: 'center', gap: '8px', color: '#999', fontSize: '0.9rem' }}>
                    <ChevronLeft size={18} /> Back to Dashboard
                </Link>
                <div style={{ fontSize: '0.9rem', fontWeight: 500 }}>{course.title}</div>
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                    <div style={{ fontSize: '0.8rem', color: '#999' }}>{progress}% complete</div>
                </div>
            </div>
