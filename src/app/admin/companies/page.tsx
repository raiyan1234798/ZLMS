...
export default function CompaniesPage() {
    ...
    const addCompanyModal = useRef<HTMLDivElement>(null);
    const editCompanyModal = useRef<HTMLDivElement>(null);
    ...
    const allCompanies = firebaseCompanies.filter(c => {
    ...
    const handleAddCompany = async () => {
        ...
        const exists = allCompanies.some(c => c.subdomain === subdomainClean) || MOCK_COMPANIES.some(c => c.subdomain === subdomainClean);
        if (exists) {
    ...
    const handleSuspendToggle = async (company: any) => {
        try {
    ...
    return (
        <div>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '32px', flexWrap: 'wrap', gap: '16px' }}>
                <div>
                    <h1 style={{ fontSize: '1.8rem', marginBottom: '4px' }}>Live Tenants</h1>
                    <p style={{ color: 'var(--text-muted)' }}>
                        {allCompanies.length} companies registered on the platform.
                        {isRealTime && <span style={{ marginLeft: '8px', color: '#10b981' }}><Wifi size={14} style={{ verticalAlign: '-2px' }} /> Live</span>}
                    </p>
                </div>
            ...
            {allCompanies.length === 0 && !loading ? (
                <div className="card" style={{ textAlign: 'center', padding: '60px 20px' }}>
                    ...
                    <h3 style={{ marginBottom: '8px' }}>No Companies Found</h3>
                    <p style={{ color: 'var(--text-muted)', marginBottom: '24px' }}>
                        Add your first company to get started with the platform.
                    </p>
                    <button className="btn-primary" onClick={() => setShowAddModal(true)}>
                        <Plus size={16} style={{ marginRight: '8px' }} /> Add Company
                    </button>
                </div>
            ) : (
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(340px, 1fr))', gap: '20px' }}>
                        ...
                        Array(3).fill(0).map((_, i) => (
                            <div key={i} className="card" style={{ padding: '24px', opacity: 0.5 }}>
                                <div style={{ display: 'flex', alignItems: 'center', gap: '14px', marginBottom: '20px' }}>
                                    ...
                                    <div>
                                        <div style={{ width: '120px', height: '16px', background: '#e2e8f0', borderRadius: '4px', marginBottom: '6px' }} />
                                        <div style={{ width: '80px', height: '12px', background: '#e2e8f0', borderRadius: '4px' }} />
                                    </div>
                                </div>
                                <div style={{ display: 'flex', gap: '8px' }}>
                                    <div style={{ width: '60px', height: '24px', background: '#e2e8f0', borderRadius: '999px' }} />
                                    <div style={{ width: '60px', height: '24px', background: '#e2e8f0', borderRadius: '999px' }} />
                                </div>
                            </div>
                        ))
                    ) : allCompanies.map((company) => {
                        const userCount = userCounts[company.id] || 0;
                        const themeColor = company.themeColor;
                        return (
                            <div key={company.id} className="card" style={{ overflow: 'hidden', padding: 0, transition: 'transform 0.2s, box-shadow 0.2s' }}
                                onMouseEnter={e => { e.currentTarget.style.transform = 'translateY(-2px)'; e.currentTarget.style.boxShadow = '0 10px 40px rgba(0,0,0,0.1)'; }}
                                ...
                                <div style={{
                                    ...
                                    <div ref={openMenuId === company.id ? menuRef : null} style={{ position: 'relative' }}>
                                        <button
                                            ...
                                            style={{ padding: '6px', borderRadius: '8px', border: '1px solid rgba(255,255,255,0.5)', background: 'rgba(255,255,255,0.8)' }}
                                        >
                                        ...
                                        {openMenuId === company.id && (
                                            <div style={{
                                                ...
                                                <button onClick={() => handleSuspendToggle(company)} style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '10px 16px', width: '100%', fontSize: '0.85rem', color: company.status === 'ACTIVE' ? '#f59e0b' : '#10b981', textAlign: 'left' }}>
                                                    {company.status === 'ACTIVE' ? <XCircle size={14} /> : <CheckCircle size={14} />}
                                ...
                                <div style={{ padding: '20px' }}>
                                    <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '16px' }}>
                                        ...
                                        <div style={{ textAlign: 'right' }}>
                                            <div style={{ fontSize: '0.75rem', color: 'var(--text-muted)', marginBottom: '4px' }}>Users</div>
                                            <div style={{ fontWeight: 600, fontSize: '0.85rem' }}>{userCount}</div>
                                        </div>
                                    </div>
                                    ...
                                    <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap', marginBottom: '16px' }}>
                                        ...
                                        {company.features.length > 4 && (
                                            <span style={{ padding: '2px 8px', borderRadius: '6px', background: 'var(--background)', fontSize: '0.7rem', color: 'var(--text-muted)' }}>
                    ...
                    {!loading && (
                        <div onClick={() => setShowAddModal(true)} className="card" style={{
                            ...
                            border: '2px dashed var(--border)', background: 'transparent', cursor: 'pointer', minHeight: '300px'
                        }}>
                            <div style={{ textAlign: 'center' }}>
                                ...
                                <div style={{ fontSize: '0.8rem', color: 'var(--text-muted)' }}>Create a new live tenant</div>
                            </div>
                        </div>
                    )}
                </div>
