...
export default function AnalyticsPage() {
    ...
    const [platformCompanies, setPlatformCompanies] = useState<any[]>([]);
    const [platformCourses, setPlatformCourses] = useState<any[]>([]);
    const [loading, setLoading] = useState(true);

    useEffect(() => {
        const fetchData = async () => {
            try {
                const [usersSnap, compsSnap, coursesSnap] = await Promise.all([
                ...
                const comps: any[] = [];
                compsSnap.forEach((doc) => comps.push({ id: doc.id, ...doc.data() }));
                setPlatformCompanies(comps);

                const courses: any[] = [];
                coursesSnap.forEach((doc) => courses.push({ id: doc.id, ...doc.data() }));
                setPlatformCourses(courses);
            } catch (error) {
    ...
    const activeCompanies = loading ? [] : platformCompanies.filter(c => activeCompanyIds.includes(c.id));
    const activeCourses = loading ? [] : platformCourses;

    const companyAnalytics = activeCompanies.map((company, idx) => {
        const users = platformUsers.filter(u => u.companyId === company.id);
        const courses = platformCourses.filter(c => c.companyId === company.id);
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
                        {companyAnalytics.map(({ company, users, courses, completionRate, engagementScore }) => (
                            <div key={company.id} style={{ display: 'flex', alignItems: 'center', gap: '20px', padding: '16px', borderRadius: '12px', border: '1px solid var(--border)' }}>
                                <div style={{ width: '44px', height: '44px', borderRadius: '10px', background: company.themeColor || '#4f46e5', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'white', fontWeight: 700, flexShrink: 0 }}>{company.name.charAt(0)}</div>
                                ...
                                <div style={{ width: '120px' }}>
                                    ...
                                    <div style={{ height: '6px', borderRadius: '999px', background: 'var(--border)' }}>
                                        <div style={{ width: `${engagementScore}%`, height: '100%', borderRadius: '999px', background: company.themeColor || '#4f46e5' }} />
                                    </div>
                                </div>
                            </div>
                        ))}
