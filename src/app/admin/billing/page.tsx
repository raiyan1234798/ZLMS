...
import { CreditCard, CheckCircle, AlertCircle, DollarSign, ArrowUpRight, TrendingUp } from 'lucide-react';

export default function BillingPage() {
    const stats = getTotalStats();
    const plans = [
        { name: 'Basic', price: 49, companies: MOCK_COMPANIES.filter(c => c.features.length < 4).length },
        { name: 'Professional', price: 149, companies: MOCK_COMPANIES.filter(c => c.features.length >= 4 && c.features.length < 5).length },
        { name: 'Enterprise', price: 499, companies: MOCK_COMPANIES.filter(c => c.features.length >= 5).length },
    ];
    ...
    return (
        <div>
            <div style={{ marginBottom: '32px' }}>
                ...
                <p style={{ color: 'var(--text-muted)' }}>Manage company subscriptions and view revenue metrics.</p>
            </div>

            {/* Revenue Stats */}
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '20px', marginBottom: '32px' }}>
                {[
                    { icon: DollarSign, label: 'Monthly Revenue', value: `$${totalMRR.toLocaleString()}`, sub: '+18% vs last month', color: '#10b981', bg: '#ecfdf5' },
                    { icon: CreditCard, label: 'Active Subscriptions', value: MOCK_COMPANIES.filter(c => c.status === 'ACTIVE').length, sub: `${stats.totalCompanies} total`, color: '#4f46e5', bg: '#eef2ff' },
                    { icon: TrendingUp, label: 'Annual Run Rate', value: `$${(totalMRR * 12).toLocaleString()}`, sub: 'Projected revenue', color: '#8b5cf6', bg: '#f5f3ff' },
                    { icon: AlertCircle, label: 'Past Due', value: 1, sub: 'NovaTech Solutions', color: '#ef4444', bg: '#fef2f2' },
                ].map((s, i) => {
                    const Icon = s.icon;
                    return (
            ...
            <div className="card" style={{ padding: 0, overflow: 'hidden' }}>
                <div style={{ padding: '20px 24px', borderBottom: '1px solid var(--border)' }}>
                    <h3 style={{ fontSize: '1rem' }}>Company Subscriptions</h3>
                </div>
