...
export default function RootPage() {
  ...
  const features = [
    ...
    { icon: <BookOpen size={32} />, title: "Course Engine", desc: "Advanced video learning, inline quizzes, and persistent course tracking.", actionText: "Explore Courses", link: "/login" },
    ...
    { icon: <LayoutDashboard size={32} />, title: "Bespoke Dashboards", desc: "Responsive, tailored analytics dashboards for administrators and learners.", actionText: "View Dashboards", link: "/login" },
  ];
    ...
    <div className="landing-root" style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', background: '#020617', color: '#f8fafc', overflowX: 'hidden' }}>
      ...
      <header className="glass-panel" style={{ padding: '20px 40px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', position: 'sticky', top: 0, zIndex: 100, background: 'rgba(2, 6, 23, 0.7) !important', borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
        ...
        <nav style={{ display: 'flex', gap: '24px', alignItems: 'center' }}>
          ...
          <Link href="/login" style={{ display: 'flex', alignItems: 'center', gap: '6px', fontWeight: 500, color: '#cbd5e1', textDecoration: 'none', transition: 'color 0.2s' }} onMouseOver={e => e.currentTarget.style.color = 'white'} onMouseOut={e => e.currentTarget.style.color = '#cbd5e1'}>
            <Lock size={14} /> Login
          </Link>
        </nav>
      </header>
                ...
                >
                  ...
                  <AnimatePresence>
                    {hoveredCompanyCard === i && (
                      ...
                      >
                        <Link href={`/login`} style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', gap: '8px', background: theme, color: 'white', padding: '14px', borderRadius: '12px', fontWeight: 600, textDecoration: 'none', transition: 'filter 0.2s', fontSize: '1rem' }} onMouseOver={e => e.currentTarget.style.filter = 'brightness(1.1)'} onMouseOut={e => e.currentTarget.style.filter = 'none'}>
